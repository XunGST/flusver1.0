...
PixCaltrain::PixCaltrain(QObject *parent,QString logFilePath,int numLine)
{
...
bool sortThroughmb1(const DoubleList &v1,const DoubleList &v2)
{
	return v1.mb1<v2.mb1;
};
void myDefinePushback(vector<DoubleList> &vecdList,const int &m1,const int &m2)
{
	DoubleList dlist;
	dlist.mb1=m1;
	dlist.mb2=m2;
...
/// <土地利用数据>
///
bool PixCaltrain::lauLoadImage2( QString _fileName )
{
	//register
	GDALAllRegister();
	//OGRRegisterAll();
...
// <公用线程>
void PixCaltrain::PixCaltrainculate()
{
...
/////////////////////为概率计算模拟重载////////////////////////////////
template<class TT> bool PixCaltrain::readData()
{
	...
	//double noData=pread->poDataset()->GetRasterBand(1)->GetNoDataValue();
...
// 		}
	...
	for (int kk=0;kk<_size;kk++)
	{
		myDefinePushback(dList,rgbLanduseType.at(kk),stastisticCount.at(kk));
	}
	sort(dList.begin(),dList.end(),sortThroughmb1);
